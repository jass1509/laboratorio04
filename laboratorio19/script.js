...
async function ej12(){
  ...
  const d = await fetch(`https://pokeapi.co/api/v2/pokemon/${id}`).then(r=>r.json());
  let html = "<ul>";
  d.stats.forEach(s=> html += `<li>${s.stat.name}: ${s.base_stat}</li>`);
  html += "</ul>";
  print(html);
}
...
function mostrar14(){
  ...
  print(html + `<br><button onclick='prev14()'>Anterior</button><button onclick='next14()'>Siguiente</button>`);
}
async function ej14(){
  if(lista14.length===0) await cargar14();
  mostrar14();
}
function next14(){ if(inicio14+3 < lista14.length){ inicio14+=3; mostrar14(); }}
function prev14(){ if(inicio14-3 >=0){ inicio14-=3; mostrar14(); }}
